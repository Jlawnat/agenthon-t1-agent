...
docker run -d \
  ...
  --network "${NETWORK}" \
  -v "$(pwd)/tests/mock_model_server_phase5d2.py:/mock_model_server.py:ro" \
  --entrypoint python \
...
for _ in $(seq 1 20); do
  if docker run --rm \
      ...
      --entrypoint python \
      finance-bench-sandbox:latest \
      -c 'import urllib.request; print(urllib.request.urlopen("http://agenthon-mock-model:8000/health", timeout=2).read().decode())' \
      >/dev/null 2>&1
  then
    break
  fi
